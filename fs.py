...
class MyFs(Operations):
	...
	def __init__(self):
		...
		chmod = self.history_decorator(self.chmod_)
		chown = self.history_decorator(self.chown_)
		truncate = self.history_decorator(self.truncate_)
		write = self.history_decorator(self.write_)
		read = self.history_decorator(self.read_)
		link = self.history_decorator(self.link_)
		unlink = self.history_decorator(self.unlink_)
		rename = self.history_decorator(self.rename_)
		symlink = self.history_decorator(self.symlink_)
		readlink = self.history_decorator(self.readlink_)
		create = self.history_decorator(self.create_)
		mkdir = self.history_decorator(self.mkdir_)
		rmdir = self.history_decorator(self.rmdir_)
		readdir = self.history_decorator(self.readdir_)
		getattr = self.history_decorator(self.getattr_)
	...
	def truncate_(self, path, length):
		pass


	def write_(self, path, data, offset, fh):
	...
	def read_(self, path, size, offset, fh):
		inode = self.get_inode(path, [S_IFREG, S_IFLNK])
		if inode is None:
			raise FuseOSError(ENOENT)
	...
	def symlink_(self, target, source): 
	...
	def getattr_(self, path, fh=None):
		node = self.get_inode(path, [S_IFDIR, S_IFREG, S_IFLNK])
		if node is None:
			raise FuseOSError(ENOENT)
	...
	# decoration of logging part of decorator
	def history_decorator(self, func):
		self.handle_fs_state("/store")
